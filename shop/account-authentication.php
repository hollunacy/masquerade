...
if (isset($_POST['register'])) {
  $statement = $pdo->prepare("SELECT * FROM user_buyers WHERE buyer_email=?");
  ...
  if ($total) {
    ...
    $statement = $pdo->prepare("INSERT INTO user_buyers (buyer_lastname, buyer_firstname, buyer_email, buyer_password, buyer_token, time_created)
                                VALUES (?,?,?,?,?,?)");
...
if (isset($_POST['login'])) {
  $statement = $pdo->prepare("SELECT * FROM user_buyers WHERE buyer_email=?");
  $statement->execute(array(strip_tags($_POST['email'])));
  $total = $statement->rowCount();
  if ($total == 0) {
    $message_error .= 'Данная почта не зарегистрирована <br>';
  } else {
    $result = $statement->fetch(PDO::FETCH_ASSOC);
    if ($result['buyer_password'] != md5(strip_tags($_POST['password']))) {
      $message_error .= 'Пароль не верный <br>';
    } else {
      $_SESSION['buyer'] = $result;
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="app-brand justify-content-center">
              <a href="products.php" class="app-brand-link gap-2">
            ...
            <form id="authentication" class="mb-3" method="POST">
              ...
              <?php if ($_GET['type'] == "register") { ?>
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="lastname" name="lastname" value="<?php if (isset($_POST['lastname'])) echo $_POST['lastname']; ?>" required>
                </div>
                <div class="mb-3">
                  <label for="firstname" class="form-label">Имя</label>
                  <input type="text" class="form-control" id="firstname" name="firstname" value="<?php if (isset($_POST['firstname'])) echo $_POST['firstname']; ?>" required>
                </div>
              <?php } ?>
              <div class="mb-3">
